/*
  File Type:     C++
  Author:        Wenyu Zhu(stupidzwy@yeah.net) 
  Creation:      2020/10/24 14:20
  Last Revision: 2020/10/24 17:05
*/
#include<git.hpp>
...
/*
 add publickey
git config --global user.name 'wenyuzhu28'
git config --global user.email 'stupidzwy@yeah.net'
ssh-keygen -t rsa -C 'stupidzwy@yeah.net'
*/
int cred_acquire_cb(git_cred **cred, const char *url, const char *username_from_url, unsigned int allowed_types,
                    void *payload) {

    git_cred_ssh_key_new(cred, username_from_url, nullptr, "C:\\Users\\stupi\\.ssh\\id_rsa", nullptr);

    return 0;
}

void git_update::set_dll_path(void) {
...
void git_update::pipeline(void) {
    ...
    // git open
    error = git_repository_open(&rep, run_path);
    if (error < 0) {
        const git_error *e = giterr_last();
        std::cerr << "Error: " << error << " / " << e->klass << " : " << e->message << std::endl;
...
//    fetch_opts.prune = GIT_FETCH_PRUNE;    // --prune
    ...
    git_signature_now(&me, "wenyuzhu28", "stupidzwy@yeah.net");

    error = git_commit_create_v(&commit_id, rep, git_reference_name(local_master), me, me, "UTF-8", "pull commit",
                                new_tree, 2, our_commit, their_commit);
...
//    auto ret=ShellExecute(NULL, NULL, task_path, NULL, NULL, SW_SHOW);
//    if((int)ret!=42)//0x2a
//        std::cerr<<"path or some other errors"<<std::endl;
...
bool git_update::run() {
    ...
    do {
        ...
        if (!CreateProcess(NULL,
                           "C:\\Users\\stupi\\Videos\\opencv-3.4.9\\samples\\cpp\\example_cmake\\cmake-build-release\\opencv_example.exe",
                           NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi)) {
            cout << "Create subprocess failed.." << GetLastError() << endl;
            system("pause"); //用于测试
            return false;
        }
//        cout << "Process ID pi.dwProcessID: " << pi.dwProcessId << endl;
//        cout << "Thread ID pi.dwThreadID : " << pi.dwThreadId << endl;
        // 等待知道子进程退出...
        WaitForSingleObject(pi.hProcess, INFINITE);//Wait subprocess exit  INFINITE
